...
def compute_and_export_image_pairs(img_pairs_path, frames,
    ...
    if not override and os.path.isfile(img_pairs_path):
        ...
        for i in range(0, len(frames) - 1, 3):
            image_pairs.extend(matcher.orb_flow(i, i + 3))
...
def compute_errors(in_vid_path, video_width, show=True, save=False):
...
def graph_AV(image_pairs, frames, cam_params_est, show=True):
    idx2pair = {}
    for p in image_pairs:
        idx2pair[(p.i, p.j)] = p
    ...
    for i in range(0, len(frames) - 1):
        p = idx2pair[(i, i+1)]
        av.append(utils.no_center_apparent_AV(p, cam_params_est[i]))
