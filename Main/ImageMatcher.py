...
class ImageMatcher:
    ...
    def orb_flow(self, f_begin, f_end) -> list[ImagePair]:

        flann = getattr(self, 'orb_flann')
        kds = getattr(self, 'orb_kds')
        tracks = [[k.pt] for k in kds[f_begin][0]]

        for i in range(f_begin + 1, f_end):
            matches = flann.knnMatch(kds[f_begin][1], kds[i][1], k=2)
            good = []
            for m in matches:
                if len(m) < 2:
                    continue
            ...
            for m in good:
                tracks[m.queryIdx].append(kds[i][0][m.trainIdx].pt)

        def valid_linear_motion(t, mag_thresh=20, dev_thresh=5):
            d = np.diff(t, axis=0)
            max_magnitude = np.linalg.norm(d, axis=1).max()
            max_deviation = np.linalg.norm(d - d.mean(axis=0), axis=1).max()
            return max_magnitude < mag_thresh and max_deviation < dev_thresh

        tracks = [t for t in tracks if len(t) == f_end - f_begin]
        tracks = [t for t in tracks if valid_linear_motion(t)]

        pairs = []
        for i in range(0, f_end - f_begin - 1):
            src_pts, dst_pts = [], []
            for t in tracks:
                src_pts.append(t[i])
                dst_pts.append(t[i + 1])
            H, src_pts, dst_pts = self.find_H(
                src_pts, dst_pts, np.ones(len(src_pts), 'b1'))
            p = ImagePair(
                self.images[f_begin + i], self.images[f_begin + i + 1],
                H, src_pts, dst_pts, f_begin + i, f_begin + i + 1, False,
            )
            pairs.append(p)
