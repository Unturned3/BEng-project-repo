...
class VideoVisualizer(QMainWindow):
    ...
    def initUI(self):
        ...
        self.showPointMarkers = QCheckBox("Show point markers", self)
        self.showPointMarkers.setChecked(True)
        self.showPointMarkers.checkStateChanged.connect(self.drawFrame)

        self.showPolyMarkers = QCheckBox("Show poly markers", self)
        self.showPolyMarkers.setChecked(True)
        self.showPolyMarkers.checkStateChanged.connect(self.drawFrame)

        self.showPoseCross = QCheckBox("Show pose cross", self)
        self.showPoseCross.checkStateChanged.connect(self.drawFrame)

        self.saveFrameButton = QPushButton("Save current frame", self)
        self.saveFrameButton.clicked.connect(self.save_cur_frame)
        ...
        r3 = QHBoxLayout()
        v1.addLayout(r3)
        r3.addWidget(self.showPoseCross)
        r3.addWidget(self.showPointMarkers)
        r3.addWidget(self.showPolyMarkers)
        r3.addWidget(self.saveFrameButton)
    ...
    def save_cur_frame(self):
        cv2.imwrite(f'{self.frame_num:05d}.png', self.cur_drawn_frame)
    ...
    def drawFrame(self):
        ...
        if self.gt_available:
            ...
            if idx in self.cpe:
                ...
                if self.showPoseCross.isChecked():
        ...
        overlay = self.poly_renderer.render(self.showPolyMarkers.isChecked(), self.showPointMarkers.isChecked())

        frame = utils.alpha_blend(overlay, frame)

        self.cur_drawn_frame = frame
    ...
    def onVideoPanelClick(self, pos):
        ...
        if self.selectedMarker is not None:
            ...
            m.updateText()
            # Find polys that contains this point and update them
            for p in self._get_QListWidget_items(self.polys_list):
                if m in p.points:
                    p.update_H()
                    world_xyzs = np.array([p.world_xyz for p in p.points], 'f4')
                    self.poly_renderer.update_poly(p.uid, world_xyzs)
        else:
            self.marker_count += 1
    ...
    def onMapPanelClick(self, pos):
        ...
        for p in self._get_QListWidget_items(self.polys_list):
            if self.selectedMarker in p.points:
                p.update_H()
        self.drawMap()
